...
\title{\textbf{What the Theory Never Specifies:\\
...
\begin{abstract}
\noindent REBUS proposes that psychedelics reduce the precision of high-level
priors, permitting belief revision; ALBUS predicts low-dose belief strengthening
(SEBUS). Neither theory has a runnable implementation. We implemented the minimal
Bayesian agent both describe, preregistering every mapping from text to
...
results. First, REBUS as literally stated produces no lasting change: dose enters
only through a precision that reverts, so it cannot appear in the terminal
belief. Second, adding consolidation rescues persistence but introduces a fork:
four defensible parameterizations of what consolidation operates on disagree
about whether dose affects the lasting outcome at all, whether integration can be
made safe, and whether entrenched beliefs are treatable; in every mechanism
tested, false insight requires more consolidation than true insight, so a safe
integration window exists. Third, SEBUS never emerges from precision
manipulations (passive relaxation, epistemic action under a correct model,
miscalibrated self-knowledge) and appears only when the single diagnostic test
carries a cost the agent declines to pay, converging with the clinical
...
assumptions the theory does not specify, and those assumptions, not the theories,
decide the clinically relevant results. Each divergence is a measurable
prediction.
\end{abstract}
...
One further literature bears directly on our central result, and we flag it now so
that the reader weighs the result correctly. The clinical account of
safety-seeking behavior holds that anxious beliefs persist because the behaviors
they motivate prevent the disconfirming test from ever being run
\cite{salkovskis1991}, and exposure therapy is understood as the deliberate
arrangement of that test \cite{foakozak1986,craske2014}. In the psychedelic
literature specifically, Wolff and colleagues \cite{wolff2020} proposed a verbal
model on which psychedelic belief relaxation enables episodes of relatively
avoidance-free exposure, and prospective data show post-psychedelic reductions in
experiential avoidance tracking reductions in depression severity
\cite{zeifman2020avoid}. The computational psychosis literature models delusion
maintenance through aberrant precision \cite{corlett2010,adams2013,sterzer2018}
but does not implement the avoidance route. So the qualitative idea that avoidance
maintains belief, and that psychedelics may act by interrupting avoidance, is not
ours and is three decades old in its clinical form. What this paper adds is the
formal result: precision mechanics alone cannot produce belief strengthening in
this model family while a single avoidance term can, and relaxation's therapeutic
role is then derivable rather than assumed, with quantitative consequences
(a cost ceiling, a dose-zero peak, a mediating variable) that the verbal accounts
do not fix.
...
low-dose prediction has no route through precision reduction and needs an avoidance
term, and that term is absent from the paper; but the crossover structure the
costly-test mechanism produces is recognizably the structure ALBUS describes. One
scope note belongs here rather than in a reviewer's report: ALBUS's own proposed
mechanism, a low-dose gain increase on priors, was deliberately excluded from the
dose mapping, because building it in produces SEBUS by construction and tests
nothing. The informative question was whether SEBUS emerges without being assumed,
and within this model family the only route that produced it was avoidance.

\textbf{Relation to the exposure account.} Read against the clinical literature
(Section~\ref{sec:related}), the costly-test result is a formal implementation of
the safety-behaviors account of belief maintenance \cite{salkovskis1991} and of
the proposal that psychedelic relaxation enables avoidance-free exposure
\cite{wolff2020}, inside REBUS's own precision vocabulary. We claim convergence,
not discovery. The formalization earns its keep in what the verbal accounts leave
open: it shows the avoidance term is necessary and not merely sufficient (three
precision-only mechanisms fail), it derives rather than posits the drug's role
(relaxation raises the epistemic value of the avoided test until it clears its
cost, which is a mechanism for Wolff and colleagues' avoidance-free exposure), and
it yields discriminating predictions the verbal accounts cannot state: engagement
with the avoided test should rise with dose where therapy works, no dose should
help above a cost ceiling, and strengthening should peak at dose zero. Each is a
measurable claim about behavior during treatment. Both theories are
...
\url{https://github.com/ArtHangup/rebus-albus-sim}, this paper is archived at
\url{https://doi.org/10.5281/zenodo.22547758}, and the repository contains the
preregistration, the amendments, all code, all result files, and the git history
that orders them. The mapping was committed at
...
relaxed beliefs under psychedelics (REBUS) to revised beliefs after
psychedelics (REBAS). \emph{Scientific Reports}, 15:3651, 2025.
...
\emph{Translational Psychiatry}, 14:394, 2024. Code and data: OSF,
DOI 10.17605/OSF.IO/EU8KH.
...
\bibitem{allohverdi2025} S.~G. Allohverdi et al. Ketamine and psilocybin
differentially impact sensory learning during the mismatch negativity.
\emph{bioRxiv}, 2025. DOI 10.1101/2025.11.06.687023.
